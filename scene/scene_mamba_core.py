...
class MambaMechanism(Scene):
    def construct(self):
        ...
        input_label = Text("Input Sequence (x)", font_size=24, color=BLUE).next_to(input_group, UP)
        
        # --- BOTTOM: 隐状态 (水箱) ---
        # 放到屏幕下方居中
        tank = Rectangle(height=2.0, width=1.5, color=WHITE).to_edge(DOWN, buff=1.0)
        tank_label = Text("Hidden State (h)", font_size=24, color=YELLOW).next_to(tank, DOWN)
        
        # 液位 (初始很低)
        fluid = Rectangle(height=0.2, width=1.5, fill_opacity=0.8, color=YELLOW, stroke_width=0)
        ...
        self.play(
            Create(input_group), Write(input_label),
            Create(tank), Write(tank_label), FadeIn(fluid),
            Create(param_box), Write(param_text),
            Create(valve), Create(arrow_1), Create(arrow_2), Create(pipe)
        )
        self.wait(0.5)
        ...
        for i, info in enumerate(token_infos):
            ...
            # 更新连线位置
            new_arrow_1 = Arrow(target_token.get_bottom(), param_box.get_top(), color=BLUE)
            ...
            # 2. 显示 Token 类型
            type_text = Text(info["type"], font_size=16, color=info["color"]).next_to(target_token, UP)
            ...
            # 5. 液体流入与状态更新
            # 流动的线
            flow = Line(valve.get_bottom(), fluid.get_top(), color=info["color"], stroke_width=flow_width)
            ...
            new_fluid = Rectangle(height=next_h, width=1.5, fill_opacity=0.8, color=YELLOW, stroke_width=0)
            new_fluid.align_to(tank, DOWN)
            
            self.play(
                Transform(fluid, new_fluid),
                run_time=0.5
            )
        ...
        # 总结
        summary = Text("Result: Only useful info is compressed into h", font_size=30, color=YELLOW).next_to(tank, RIGHT, buff=1)
